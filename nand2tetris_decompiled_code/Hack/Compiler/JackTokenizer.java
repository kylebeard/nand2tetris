...
public class JackTokenizer
{
    ...
    public JackTokenizer(final Reader r) {
        try {
            (this.parser = new StreamTokenizer(r)).parseNumbers();
    ...
    public void advance() {
        try {
            switch (this.parser.ttype) {
                ...
                case -3: {
                    final String sval = this.parser.sval;
                    if (this.keywords.containsKey(sval)) {
                        this.tokenType = 1;
                        this.keyWordType = (int)this.keywords.get(sval);
                        break;
                    }
                    this.tokenType = 3;
    ...
    public boolean hasMoreTokens() {
        final int ttype = this.parser.ttype;
        final StreamTokenizer parser = this.parser;
        return ttype != -1;
    }
    ...
    private void initKeywords() {
        (this.keywords = new Hashtable()).put("class", new Integer(1));
    ...
    private void initSymbols() {
        (this.symbols = new Hashtable()).put("(", "(");
        this.symbols.put(")", ")");
        this.symbols.put("[", "[");
        this.symbols.put("]", "]");
        this.symbols.put("{", "{");
